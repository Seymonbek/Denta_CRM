...
class TestReadApi:
    def _seed(self, head_doctor, doctor, patient):
        with patch("apps.notifications.tasks.send_notification.delay"):
...
class TestInventoryLowStockIntegration:
    ...
    def test_low_stock_via_usage_fires_notification(
        ...
        from apps.doctors.models import CommissionBasis, DoctorProfile, WorkingHours
        ...
        alert = alerts.first()
        assert alert.status in {NotificationStatus.PENDING, NotificationStatus.SENT, NotificationStatus.FAILED}
