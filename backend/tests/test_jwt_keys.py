...
def test_token_signed_with_old_key_rejected_after_rotation(user):
    ...
    old_state_backend = state.token_backend
    old_token_backend = Token.token_backend

    new_backend = TokenBackend(
        algorithm="HS256",
        signing_key="brand-new-rotated-key-with-more-than-32-bytes-of-entropy-here",
        verifying_key="brand-new-rotated-key-with-more-than-32-bytes-of-entropy-here",
    )
    try:
        state.token_backend = new_backend
        Token.token_backend = new_backend
        resp = api_client.get(reverse("v1:accounts:me"))
        assert resp.status_code == status.HTTP_401_UNAUTHORIZED, (
            "Token minted under the old key must be rejected after rotation."
        )
    finally:
        state.token_backend = old_state_backend
        Token.token_backend = old_token_backend
...
def test_env_var_names_are_documented_in_env_examples():
    ...
    assert "JWT_SIGNING_KEY" in backend_content, (
        "T124: JWT_SIGNING_KEY missing from .env.example"
    )
