...
class TestSelectors:
    ...
    def test_filter_is_sent(self, treatment, head_doctor):
        ...
        sent_only = list(filter_prescriptions(treatment_id=treatment.pk, is_sent=True))
        pending_only = list(filter_prescriptions(treatment_id=treatment.pk, is_sent=False))
...
class TestPrescriptionListAPI:
    ...
    def test_filter_is_sent(self, api_client, head_doctor, treatment):
        ...
        r = _auth(api_client, head_doctor).get(f"{PRESCRIPTIONS_URL}?treatment={treatment.pk}&is_sent=true")
        assert r.status_code == status.HTTP_200_OK
        assert r.json()["count"] == 1
